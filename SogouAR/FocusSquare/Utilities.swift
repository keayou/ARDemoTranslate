...
extension HitTestRay: _ObjectiveCBridgeable {

    typealias _ObjectiveCType = HitTestRayOC
    static func _isBridgedToObjectiveC() -> Bool {
        return true
    }
    // 获取转换的目标类型
    static func _getObjectiveCType() -> Any.Type {
        return HitTestRayOC.self
    }
    func _bridgeToObjectiveC() -> HitTestRayOC {
        return HitTestRayOC(origin: origin, direction: direction)
    }
    
    static func _forceBridgeFromObjectiveC(_ source: HitTestRayOC, result: inout HitTestRay?) {
        result = HitTestRayOC(origin: source.origin, direction: source.direction)! as HitTestRay
    }
    
    static func _conditionallyBridgeFromObjectiveC(_ source: HitTestRayOC, result: inout HitTestRay?) -> Bool {
        _forceBridgeFromObjectiveC(source, result: &result)
        return true
    }
    
    static func _unconditionallyBridgeFromObjectiveC(_ source: HitTestRayOC?) -> HitTestRay {
        return HitTestRay(origin: source!.origin, direction: source!.direction)
    }
...
extension FeatureHitTestResult: _ObjectiveCBridgeable {
    
    typealias _ObjectiveCType = FeatureHitTestResultOC

    func _bridgeToObjectiveC() -> FeatureHitTestResultOC {
        return FeatureHitTestResultOC(position: position,
    ...
    static func _forceBridgeFromObjectiveC(_ source: FeatureHitTestResultOC, result: inout FeatureHitTestResult?) {
        result = FeatureHitTestResultOC(position: source.position,
                                        distanceToRayOrigin: source.distanceToRayOrigin,
                                        featureHit: source.featureHit,
                                        featureDistanceToHitResult: source.featureDistanceToHitResult)! as FeatureHitTestResult
    }
    
    static func _conditionallyBridgeFromObjectiveC(_ source: FeatureHitTestResultOC, result: inout FeatureHitTestResult?) -> Bool {
        _forceBridgeFromObjectiveC(source, result: &result)
        return true
    }
    
    static func _unconditionallyBridgeFromObjectiveC(_ source: FeatureHitTestResultOC?) -> FeatureHitTestResult {
        return FeatureHitTestResult(position: (source?.position)!,
                                    distanceToRayOrigin: (source?.distanceToRayOrigin)!,
                                    featureHit: (source?.featureHit)!,
                                    featureDistanceToHitResult: (source?.featureDistanceToHitResult)!)
...
extension ScreenPositionResult: _ObjectiveCBridgeable {
    typealias _ObjectiveCType = ScreenPositionResultOC

    func _bridgeToObjectiveC() -> ScreenPositionResultOC {
        return ScreenPositionResultOC(position: position,
                                      planeAnchor: planeAnchor,
                                      hitAPlane: hitAPlane)
    }
    
    static func _forceBridgeFromObjectiveC(_ source: ScreenPositionResultOC, result: inout ScreenPositionResult?) {
        result = ScreenPositionResultOC(position: source.position,
                                        planeAnchor: source.planeAnchor,
                                        hitAPlane: source.hitAPlane)! as ScreenPositionResult
    }
    
    static func _conditionallyBridgeFromObjectiveC(_ source: ScreenPositionResultOC, result: inout ScreenPositionResult?) -> Bool {
        _forceBridgeFromObjectiveC(source, result: &result)
        return true
    }
    
    static func _unconditionallyBridgeFromObjectiveC(_ source: ScreenPositionResultOC?) -> ScreenPositionResult {
        return ScreenPositionResult(position: (source?.position)!,
                                    planeAnchor: (source?.planeAnchor)!,
                                    hitAPlane: (source?.hitAPlane)!)
    }
}
...
extension ARSCNView {
    ...
    func worldPositionFromScreenPosition(_ position: CGPoint,
                                         _ objectPos: SCNVector3,
                                         _ infinitePlane: Bool) -> ScreenPositionResult {
        ...
        if let result = planeHitTestResults.first {
            ...
            return ScreenPositionResult(position: planeHitTestPosition, planeAnchor: (planeAnchor as? ARPlaneAnchor)!, hitAPlane: true)
        }
        ...
        if (infinitePlane && dragOnInfinitePlanesEnabled) || !highQualityFeatureHitTestResult {
            ...
            let pointOnPlane = objectPos

            let pointOnInfinitePlane = hitTestWithInfiniteHorizontalPlane(position, pointOnPlane)
            if pointOnInfinitePlane != nil {
                return ScreenPositionResult(position: pointOnInfinitePlane!, planeAnchor: nil, hitAPlane: true)
        ...
        if highQualityFeatureHitTestResult {
            return ScreenPositionResult(position: featureHitTestPosition!, planeAnchor: nil, hitAPlane: false)
        ...
        if !unfilteredFeatureHitTestResults.isEmpty {
            ...
            return ScreenPositionResult(position: result.position, planeAnchor: nil, hitAPlane: false)

            //            return (result.position, nil, false)
        }
        return ScreenPositionResult(position: SCNVector3Zero, planeAnchor: nil, hitAPlane: false)
